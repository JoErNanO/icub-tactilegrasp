...
const string TactileGrasp::COMMAND_LIST[]  = {
	"control", "oh", "calibrate",
	"gt", "gs", "grasp compliant", 
...
{    
	...
	* specifically the port names which are dependent on the module name*/
    ...
    optCart.put("remote", ("/" + robotName + "/cartesianController/" + whichArm + "_arm").c_str());
    optCart.put("local", ("/cartesian_client/" + whichArm + "_arm").c_str());
    ...
    //Cartesian::limitTorsoPitch(iCart, maxTorsoPitch);
    ...
    /* ******* Start the threads.                                   ******* */
    // Gaze thread to look at action
    gazeThread = new GazeThread(100, iCart, iGaze);
    gazeThread->start();
...
bool TactileGrasp::interruptModule(){
	...
	// Cartesian controller stuff
    if (iCart) {
        // we require an immediate stop before closing the clientCart for safety reason
        iCart->stopControl();
        // restore the controller context as it was before opening the module
        iCart->restoreContext(startup_context_id_cart);
        // to restore the center of the palm as actual end-effector
...
bool TactileGrasp::close() {
    /* ******* Stop the threads.                    ******* */
    // Gaze thread
    if (gazeThread && gazeThread->isRunning()) {
        gazeThread->stop();
    }
    // Grasp thread
