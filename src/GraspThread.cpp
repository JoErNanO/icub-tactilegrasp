...
bool GraspThread::threadInit() {
    ...
    touch_threshold	= 2;					// default touch threshold
    ...
    startPos.resize(NUM_JOINTS);
    doControl = false;


    // Store initial robot position
    encs->getEncoders(startPos.data());
...
void GraspThread::threadRelease() {
    ...
    // Open the hand
    setArmJointsPos();
    // Go back to robot position before module was started
    pos->positionMove(startPos.data());
...
void GraspThread::grasp() {
    ...
    for (int i = 0; i < touchPerFinger.size(); ++i) {
        cout << touchPerFinger[i] << " ";
    }
    cout << "\n";
#endif
...
//	pos->positionMove(7 , 15);
//	pos->positionMove(8 , 30);
//	pos->positionMove(9 ,  0);
//	pos->positionMove(10,  0);
	
    pos->positionMove(0 ,-25);
    pos->positionMove(1 , 35);
    pos->positionMove(2 , 18);
    pos->positionMove(3 , 86);
    pos->positionMove(4 ,-32);
    pos->positionMove(5 ,  9);
    pos->positionMove(6 , 11);
    pos->positionMove(7 , 40);
    pos->positionMove(8 , 60);
    pos->positionMove(9 , 30);
    pos->positionMove(10, 30);
    //-24.197802 34.694505 17.658899 86.110989 -32.203575 8.915561 10.768514 11.678831 10.661539 3.306115 11.68853 4.109577 12.442388 0.000012 5.317948 24.625356

    // set the fingers to the original position
    if (rightHand) {
	pos->positionMove(11, 5);
	pos->positionMove(12, 35);
	pos->positionMove(13, 15);
...
bool GraspThread::setTouchThreshold(int newTouchThreshold) {
    cout << moduleName << "Current threshold: " << touch_threshold << " ";
	...
	touchThresholdSem.post();
        cout << "Updated threshold: " << touch_threshold << "\n";
        return true;
    }
   
    cout << moduleName << "Nothing changed, threshold: " << touch_threshold << "\n";
